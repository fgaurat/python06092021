...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.todo_table.heading(
            'completed', text='completed', anchor=tk.CENTER)

        mes_todos = self._dao.find_all()

        for todo in mes_todos:
            self.todo_table.insert(parent='', index=todo.id, iid=todo.id, text='', values=(
...
def main():
    parser = argparse.ArgumentParser(
        description='Insert todos from web in sqlite')
